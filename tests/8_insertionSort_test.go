...
func TsetInsertionSort(t *testing.T) {
	testData := []insertionSort{
	...
	for _, v := range testData {
		assert.Equal(t, v.want, alg.InsertionSort(v.list))
	}
}

func TsetInsertionSortDesc(t *testing.T) {
	testData := []quikSort{
	...
	for _, v := range testData {
		assert.Equal(t, v.want, alg.InsertionSortDesc(v.list))
	}
}
